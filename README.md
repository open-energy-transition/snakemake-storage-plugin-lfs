...
> **Breaking change from v0.3:** The previous URL format `lfs://{oid}/{path}` (where `{oid}` was the 64-character SHA-256 LFS object ID) is no longer supported. Migrate by replacing the OID with the git ref (tag or commit) at which you want to pin the file.
